...
- Most `ViewProps` (such as `testID`, `accessibilityLabel`, `pointerEvents`) are not forwarded; only `style` is.
- On iOS, `onPageScroll` and `onPageScrollStateChanged` only fire on iOS 18+.

See [Platform behavior](#platform-behavior) and [Compatibility notes](#compatibility-notes) for the full list.
...
`PagerView` exposes a ref with `setPage`, `setPageWithoutAnimation`, and `setScrollEnabled`.

```tsx ImperativePagerView.tsx
import { useRef } from 'react';
import { Button, StyleSheet, Text, View } from 'react-native';
import PagerView, { type PagerViewRef } from '@expo/ui/community/pager-view';

export default function ImperativePagerView() {
  const pagerRef = useRef<PagerViewRef>(null);
...
> When you use `setScrollEnabled` imperatively, omit the `scrollEnabled` prop. If the prop is also provided, subsequent prop changes win and reset the value you set imperatively.
...
| Feature                                     | Android                | iOS                                                                                                       |
| ------------------------------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------- |
| Minimum platform version                    | Any supported version  | iOS 17                                                                                                    |
...
- Unlike upstream `react-native-pager-view`, `setScrollEnabled` triggers a re-render so the new value flows through to the native view as a prop. It is still useful for toggling from non-React contexts such as a ref-based gesture handler.
...
- The `usePagerView` hook from upstream is not provided. Use a `ref` to `PagerView` to access `setPage`, `setPageWithoutAnimation`, and `setScrollEnabled`.
